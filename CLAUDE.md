...
國中校園點名系統 (a junior-high-school attendance system). Full business context, permission model, and database ERD live in [system_structure.md](system_structure.md) — read it before touching schema, roles, or attendance business logic. Accounts/roles, school classes/students/teachers, and the core attendance recording flow (`attendance_sessions`/`attendance_records`) are implemented; the follow-up/audit-log domain (`attendance_follow_ups`, activitylog integration) and the live status dashboard are not yet built (see system_structure.md's 開發流程規劃 for the phase order).
