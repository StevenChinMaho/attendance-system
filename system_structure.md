...
- 本機測試常常需要 `migrate:fresh` 清空資料庫，手動在畫面上重建班級／導師／副班長很花時間——`sail artisan migrate:fresh --seed`（或 `migrate --seed`）除了角色權限，也會自動跑 `Database\Seeders\DemoDataSeeder` 建立一組固定帳號可以直接登入測試：`teacher1`／`teacher2`（導師，各帶一班）、`rep1`／`rep2`（副班長），密碼統一是 `password`；班級固定建在「目前」學年度／學期，開啟畫面不用先切換學年度就看得到。這個 seeder 只在 local/testing 環境跑（見 `DatabaseSeeder`），不會混進正式環境。
...
即時狀態看板 (Dashboard)：為導師與生教組設計了直覺的視覺化看板。每個班級一列，早/中/下午三個時段同時並排顯示（不用像點名頁一樣切換時段才看得到），每個時段各自統計「出席」（含遲到，人還是有到校）跟「缺席」（含早退，提早離校）兩個數字——四種出席狀態對這個總覽畫面而言只需要分成「到了沒」的二分法，見 `AttendanceStatus::countsAsPresent()`。右側「需留意學生」只列真正缺席或早退的學生（遲到不算例外），滑鼠移到學生名字上會彈出當次記錄的處理情形。看板採輪詢（polling）方式更新即可，不需要 WebSocket。

3. 導覽列統一的「點名」入口

副班長、導師、管理者三種角色共用同一個導覽列「點名」入口，不再各自分散——管理者也不需要另外從「班級管理」列表點進去才能點名。差別只在於「可以選哪些班級」：副班長/導師只看得到自己名下的班級（見 `App\Models\User::ownSchoolClasses()`），管理者沒有固定班級，看的是「目前選取」學年度／學期裡的全部班級。只有一個班可選時顯示單一連結，超過一個班顯示下拉選單，介面邏輯三種角色共用一份（`App\Livewire\AttendanceQuickLink`）。這個選單／連結跟著 nav bar 的學年度篩選走，只顯示目前選取範圍內的班級——過去帶過的班級不會因為換帶新班而被自動收回存取權，仍可透過切換學年度或直接網址回頭補登、更正點名紀錄，只是不會預設出現在快捷選單裡；管理者沒有「應該有一個預設班級」的期待，目前選取範圍內沒有任何班級時就不顯示這個項目。
