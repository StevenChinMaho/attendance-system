...
導師理論上可能同時（或跨學期）帶超過一個班（`teachers.id` 對 `school_classes.homeroom_teacher_id` 是一對多），這種情況下導覽列的「點名」不會是單一連結，而是切換成下拉選單列出這個帳號在「目前選取」學年度／學期裡的班級，選了哪個班就進哪個班的點名頁；只有一個班的帳號（多數副班長／導師的情況）維持原本單一連結的簡單體驗（見 `App\Models\User::ownSchoolClasses()`）。這個選單／連結跟著 nav bar 的學年度篩選走，只顯示目前選取範圍內的班級——過去帶過的班級不會因為換帶新班而被自動收回存取權，仍可透過切換學年度或直接網址回頭補登、更正點名紀錄，只是不會預設出現在快捷選單裡。
