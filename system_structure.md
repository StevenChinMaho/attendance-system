...
- 帳號僅能由管理者於後台建立，不開放自主註冊；`users` 保留 `is_active` 欄位供管理者停用帳號而不需刪除資料。帳號、老師、班級都支援編輯與刪除，但刪除都有對應的保護：帳號不能刪除自己、不能刪除系統裡最後一個管理者、有點名或處理情形操作紀錄的帳號不能刪除（改用停用）；老師仍在擔任某班導師時不能刪除；班級已經有學生或點名紀錄時不能刪除（避免整串歷史紀錄被連坐刪除），只用來清掉建錯的空班級。
