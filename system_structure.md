...
導師理論上可能同時（或跨學期）帶超過一個班（`teachers.id` 對 `school_classes.homeroom_teacher_id` 是一對多），這種情況下導覽列的「點名」不會是單一連結，而是切換成下拉選單列出這個帳號名下所有的班級，選了哪個班就進哪個班的點名頁；只有一個班的帳號（多數副班長／導師的情況）維持原本單一連結的簡單體驗（見 `App\Models\User::ownSchoolClasses()`）。過去帶過的班級不會因為換帶新班而被自動收回存取權，仍可回頭補登或更正點名紀錄。
...
切換方式是下拉式選單（不開放自由輸入學年度／學期數字），選項範圍從資料庫最舊一筆班級紀錄的學年度開始，到目前學年度後推 10 年；選擇的結果存在 session，屬於「使用者現在想看哪個學年度」的個人瀏覽狀態，不影響其他使用者、也不需要新增資料表欄位。班級管理頁面與即時看板都遵循這個篩選——新增班級介面雖然仍會顯示學年度／學期欄位，但鎖定為目前選取的值、不能自由輸入，避免瀏覽某個學年度列表時手滑把新班級建到別的學年度去；編輯既有班級不受此限制，仍可自由調整（例如更正資料建檔錯誤）。
